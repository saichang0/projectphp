<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="https://cdn.tailwindcss.com"></script>
    <title>Document</title>
</head>

<body>
    <aside class="fixed top-16 left-0 w-64 h-screen bg-white shadow-md">
        <nav class="px-4 pl-10 mt-1">
            <ul class="space-y-2">
                <li>
                    <a href="/project/overview.php" class="flex items-center p-2 rounded-lg hover:bg-gray-200 gap-2" default>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-house-gear" viewBox="0 0 16 16">
                            <path d="M7.293 1.5a1 1 0 0 1 1.414 0L11 3.793V2.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v3.293l2.354 2.353a.5.5 0 0 1-.708.708L8 2.207l-5 5V13.5a.5.5 0 0 0 .5.5h4a.5.5 0 0 1 0 1h-4A1.5 1.5 0 0 1 2 13.5V8.207l-.646.647a.5.5 0 1 1-.708-.708z" />
                            <path d="M11.886 9.46c.18-.613 1.048-.613 1.229 0l.043.148a.64.64 0 0 0 .921.382l.136-.074c.561-.306 1.175.308.87.869l-.075.136a.64.64 0 0 0 .382.92l.149.045c.612.18.612 1.048 0 1.229l-.15.043a.64.64 0 0 0-.38.921l.074.136c.305.561-.309 1.175-.87.87l-.136-.075a.64.64 0 0 0-.92.382l-.045.149c-.18.612-1.048.612-1.229 0l-.043-.15a.64.64 0 0 0-.921-.38l-.136.074c-.561.305-1.175-.309-.87-.87l.075-.136a.64.64 0 0 0-.382-.92l-.148-.044c-.613-.181-.613-1.049 0-1.23l.148-.043a.64.64 0 0 0 .382-.921l-.074-.136c-.306-.561.308-1.175.869-.87l.136.075a.64.64 0 0 0 .92-.382zM14 12.5a1.5 1.5 0 1 0-3 0 1.5 1.5 0 0 0 3 0" />
                        </svg>
                        Overview
                    </a>
                </li>
                <li>
                    <a href="/project/document.php" class="flex items-center p-2 rounded-lg hover:bg-gray-200 gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-earmark-code" viewBox="0 0 16 16">
                            <path d="M14 4.5V14a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2h5.5zm-3 0A1.5 1.5 0 0 1 9.5 3V1H4a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V4.5z" />
                            <path d="M8.646 6.646a.5.5 0 0 1 .708 0l2 2a.5.5 0 0 1 0 .708l-2 2a.5.5 0 0 1-.708-.708L10.293 9 8.646 7.354a.5.5 0 0 1 0-.708m-1.292 0a.5.5 0 0 0-.708 0l-2 2a.5.5 0 0 0 0 .708l2 2a.5.5 0 0 0 .708-.708L5.707 9l1.647-1.646a.5.5 0 0 0 0-.708" />
                        </svg>
                        Documents
                    </a>
                </li>
            </ul>
        </nav>
    </aside>

    <script>
        document.querySelectorAll("aside nav ul li a").forEach(link => {
            link.addEventListener("click", function(e) {
                // Prevent the page from reloading
                e.preventDefault();

                // Remove the active class from all links
                document.querySelectorAll("aside nav ul li a").forEach(l => {
                    l.classList.remove("text-orange-500", "font-semibold", "bg-gray-200");
                });

                // Add active class to the clicked link
                this.classList.add("text-orange-500", "font-semibold", "bg-gray-200");

                // Save the active link to localStorage
                localStorage.setItem("activeNavLink", this.getAttribute("href"));

                // Optionally, navigate to the link after style change
                window.location.href = this.getAttribute("href");
            });
        });

        window.addEventListener("DOMContentLoaded", () => {
            const activeHref = localStorage.getItem("activeNavLink");
            if (activeHref) {
                const activeLink = document.querySelector(`aside nav ul li a[href='${activeHref}']`);
                if (activeLink) {
                    activeLink.classList.add("text-orange-500", "font-semibold", "bg-gray-200");
                }
            }
        });
    </script>

</body>

</html>
